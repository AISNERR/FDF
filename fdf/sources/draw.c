...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/02 20:40:10 by aisner            #+#    #+#             */
/*   Updated: 2022/04/03 03:51:19 by aisner           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_line(t_point f, t_point s, t_fdf *fdf)
{
	...
	delta.x = helper03(f, s);
	delta.y = helper04(f, s);
	sign.x = helper01(f, s);
	sign.y = helper02(f, s);
	...
	while (cur.x != s.x || cur.y != s.y)
	{
		...
		if (error[1] < delta.x)
			cur.y += sign.y;
		error[0] = error_checker(error, delta);
	}
}
